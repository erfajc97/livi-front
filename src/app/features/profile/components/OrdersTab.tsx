...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
  ...
  order_cancelled: { label: 'Cancelado', color: 'bg-red-100 text-red-700' },
...
export default function OrdersTab() {
  ...
  // Detail view
  if (selected) {
    const s = STATUS_MAP[selected.status] || { label: selected.status, color: 'bg-gray-100 text-gray-700' };
    return (
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400">{new Date(selected.createdAt).toLocaleDateString('es-EC', { dateStyle: 'long' })}</p>
          </div>
            ...
            <div key={item.id} className="flex gap-3 items-center p-3">
              <div className="w-12 h-12 rounded-lg bg-gray-50 overflow-hidden shrink-0">
                {item.productImage ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-gray-300 text-[9px]">Sin img</div>
                )}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-black truncate">{item.productName || 'Producto'}</p>
                <p className="text-xs text-gray-400">{item.mlSize ? `${item.mlSize}ml ${item.isFullBottle ? 'Botella' : 'Decant'}` : ''} · {item.quantity}x</p>
              </div>
        ...
        {/* Status Timeline */}
        {history.length > 0 && (
          <div className="rounded-2xl border border-gray-100 bg-linear-to-b from-gray-50 to-white p-5">
            <h3 className="text-sm font-bold text-black mb-4">Seguimiento de tu pedido</h3>
                ...
                return (
                  <div key={entry.id} className={`relative pb-5 last:pb-0 ${isLast ? '' : ''}`}>
                    <div className={`absolute -left-8 top-0 w-6 h-6 rounded-full flex items-center justify-center text-xs ${
                      isLast ? 'bg-accent shadow-md shadow-accent/30' : isFirst ? 'bg-gray-200' : 'bg-gray-100'
                    }`}>
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-gray-400 mt-0.5">Revisa el estado de tus pedidos</p>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {awaitingTracking && (
                    <p className="text-[11px] text-amber-600 mt-1">Guía pendiente de asignación</p>
                  )}
                </div>
                <div className="flex flex-col items-center gap-1 shrink-0">
                  <div className="w-8 h-8 rounded-full bg-gray-100 group-hover:bg-black flex items-center justify-center transition-colors">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-gray-400 group-hover:text-white transition-colors"><polyline points="9 18 15 12 9 6" /></svg>
                  </div>
                  <span className="text-[9px] text-gray-400 group-hover:text-black font-medium transition-colors">Ver</span>
                </div>
              </button>
            );
          })}
