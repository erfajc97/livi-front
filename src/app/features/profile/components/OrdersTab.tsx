  ...
  if (selected) {
    ...
    return (
      <div className="space-y-5">
        <button onClick={() => setSelected(null)} className="flex items-center gap-2 text-sm text-text-muted hover:text-black transition-colors">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-text-muted">{new Date(selected.createdAt).toLocaleDateString('es-EC', { dateStyle: 'long' })}</p>
          </div>
        ...
        <div className="rounded-xl border border-border divide-y divide-border">
          {selected.items.map((item) => (
            <div key={item.id} className="flex gap-3 items-center p-3">
              <div className="w-12 h-12 rounded-lg bg-surface-raised overflow-hidden shrink-0">
                {item.productImage ? (
        ...
        {/* Status Timeline */}
        {history.length > 0 && (
          <div className="rounded-2xl border border-border bg-linear-to-b from-gray-50 to-white p-5">
              ...
              {history.map((entry, i) => {
                ...
                return (
                  <div key={entry.id} className={`relative pb-5 last:pb-0 ${isLast ? '' : ''}`}>
                    ...
                    <div>
                      <p className={`text-sm font-semibold ${isLast ? 'text-black' : 'text-text-muted'}`}>{label}</p>
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-text-muted mt-0.5">Revisa el estado de tus pedidos</p>
      </div>
          ...
          {orders.map((order) => {
            ...
            return (
              <button
                ...
                className="group w-full flex items-center gap-4 rounded-xl border border-border p-4 text-left hover:border-black hover:shadow-sm transition-all cursor-pointer"
              >
                <div className="w-14 h-14 rounded-lg bg-surface-raised overflow-hidden shrink-0">
                ...
                <div className="flex flex-col items-center gap-1 shrink-0">
                  <div className="w-8 h-8 rounded-full bg-surface-raised group-hover:bg-black flex items-center justify-center transition-colors">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-text-muted group-hover:text-white transition-colors"><polyline points="9 18 15 12 9 6" /></svg>
                  </div>
                  <span className="text-xs text-text-muted group-hover:text-black font-medium transition-colors">Ver</span>
                </div>
              </button>
            );
          })}
