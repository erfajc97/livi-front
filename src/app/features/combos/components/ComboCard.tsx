...
export default function ComboCard({ combo }: ComboCardProps) {
  ...
  return (
    <a href={`/combo/${combo.id}`} className="group relative flex flex-col bg-white border border-gray-200 rounded-xl overflow-hidden w-full hover:shadow-md transition-shadow">
      {/* Discount badge */}
      {discountPercent > 0 && (
        <div className="absolute top-3 left-3 z-10 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
          -{discountPercent}%
        </div>
      )}
      ...
      <div className="flex flex-col gap-2 p-4">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <ul className="space-y-0.5">
              {products.slice(0, 4).map((cp) => {
                ...
                return (
                  <li key={cp.id} className="text-[11px] text-gray-600 leading-tight">
        ...
        <div className="flex flex-col gap-1 mt-1">
          {hasDiscount && (
            <span className="font-body text-xs text-red-400 line-through">
              {formatCurrency(combo.finalPrice)}
            </span>
          )}
          <span className={`font-heading text-lg font-bold ${hasDiscount ? 'text-green-600' : 'text-gray-900'}`}>
