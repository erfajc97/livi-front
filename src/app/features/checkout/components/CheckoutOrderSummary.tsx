            ...
            <div key={item.variantId} className="flex gap-4 items-center">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center justify-between mt-2">
                  {item.comboId ? (
                    <span className="text-[11px] font-bold text-black select-none">Cant: {item.quantity}</span>
                  ) : (
                    <div className="flex items-center gap-0">
                      <button type="button" onClick={() => updateQty(item.variantId, item.quantity - 1)} className="w-6 h-6 flex items-center justify-center border border-gray-300 text-gray-500 hover:text-black rounded-l text-xs">−</button>
                      <span className="w-7 h-6 flex items-center justify-center border-y border-gray-300 text-[11px] font-bold text-black select-none">{item.quantity}</span>
                      <button type="button" onClick={() => updateQty(item.variantId, item.quantity + 1)} className="w-6 h-6 flex items-center justify-center border border-gray-300 text-gray-500 hover:text-black rounded-r text-xs">+</button>
                    </div>
                  )}
        ...
        <div className="mb-6">
          ...
          {couponApplied ? (
            <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-4 py-3">
              <div>
                <span className="font-mono font-bold text-green-700 text-sm">{couponCode}</span>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 border border-gray-300 rounded-lg px-4 py-2.5 text-sm font-mono uppercase text-black placeholder:text-gray-400 focus:outline-none focus:border-black focus:ring-1 focus:ring-black"
              />
              <button
                type="button"
                onClick={onApplyCoupon}
                disabled={couponLoading || !couponCode.trim()}
                className="px-5 py-2.5 bg-black text-white text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-gray-800 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
